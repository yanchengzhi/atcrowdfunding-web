					...
					<div class="panel-body">
						<form role="form">
							<div class="form-group">
								<label for="exampleInputPassword1">许可名称</label> <input
									type="text" class="form-control" value="${permission.name}" id="perName"
									placeholder="请输入许可名称">
							</div>
							<div class="form-group">
								<label for="exampleInputEmail1">链接地址</label> <input type="email"
									class="form-control" value="${permission.url}" id="url" placeholder="请输入链接地址">
							</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#updateBtn').click(function(){
				...
				if(perName == ""){
					layer.msg("许可名称不能为空！",{time:2000,icon:0,shift:5},function(){ 
					});
					return;
				}
				var url = $('#url').val();
				if(url == ""){
					layer.msg("链接地址不能为空！",{time:2000,icon:0,shift:5},function(){ 
					});
					return;
				}
				...
				$.ajax({
				    ...
				    success:function(result){
				    	...
				    	if(result.success){
							layer.msg("许可信息修改成功！",{time:2000,icon:6,shift:5},function(){ 
							});
							window.location.href="${APP_PATH}/permission/index";
				    	}else{
							layer.msg("许可信息修改失败！",{time:2000,icon:5,shift:5},function(){ 
							});	
				    	}
				    }
				});
