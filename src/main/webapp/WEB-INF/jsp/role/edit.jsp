					...
					<li style="padding-top: 8px;">
						<div class="btn-group">
							<button type="button"
								...
								<i class="glyphicon glyphicon-user"></i> ${currentUser.username}<span class="caret"></span>
							</button>
						...
						<form role="form">
							<div class="form-group">
								<label for="exampleInputPassword1">名称</label> <input
									type="text" class="form-control" id="name" value="${role.name}"
									placeholder="请输入名称">
							</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#updateBtn').click(function(){
				...
				var name = $('#name').val();
				if(name == ""){
					layer.msg("名称不能为空！",{time:2000,icon:0,shift:5},function(){ 
					});
					return;
				}
				...
				$.ajax({
				    ...
				    success:function(result){
				    	...
				    	if(result.success){
							layer.msg("用户修改成功！",{time:2000,icon:6,shift:5},function(){ 
							});
							window.location.href="${APP_PATH}/role/index";
				    	}else{
							layer.msg("用户修改失败！",{time:2000,icon:5,shift:5},function(){ 
							});	
				    	}
				    }
				});
