						...
						<input type="hidden" name="userId" value="${user.id}"/>
							...
							<div class="form-group">
								<ul>
									...
									<br>
									<li id="rtlBtn" class="btn btn-default glyphicon glyphicon-chevron-left"
	...
	var loadingIndex = null;
		$(function() {
			...
			$('#ltrBtn').click(function(){
				...
				if(opts.length==0){
					layer.msg("请选择要分配的角色！",{time:3000,icon:5,shift:5},function(){
					...
					$.ajax({
						...
						success:function(result){
							if(result.success){
								...
								layer.msg("角色分配成功！",{time:3000,icon:6,shift:5},function(){
									
								});
							}else{
								layer.msg("角色分配失败！",{time:3000,icon:5,shift:5},function(){
			...
			$('#rtlBtn').click(function(){
				...
				if(opts.length==0){
					layer.msg("请选择要取消分配的角色！",{time:3000,icon:5,shift:5},function(){
					...
					$.ajax({
						...
						success:function(result){
							if(result.success){
								...
								layer.msg("取消角色分配成功！",{time:3000,icon:6,shift:5},function(){
									
								});
							}else{
								layer.msg("取消角色分配失败！",{time:3000,icon:5,shift:5},function(){
