						...
						<form role="form" id="userForm">
							<div class="form-group">
								<label for="exampleInputPassword1">登陆账号</label> <input
									type="text" class="form-control" value="${user.loginName}" id="loginName"
									placeholder="请输入登陆账号">
							</div>
							<div class="form-group">
								<label for="exampleInputPassword1">用户名称</label> <input
									type="text" class="form-control" value="${user.username}" id="username"
									placeholder="请输入用户名称">
							</div>
							<div class="form-group">
								<label for="exampleInputEmail1">邮箱地址</label> <input type="email"
									class="form-control" id="email" value="${user.email}" placeholder="请输入邮箱地址">
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#resetBtn').click(function(){
				//将jquery对象转换为DOM对象,调用DOM对象的reset方法
			...
			$('#updateBtn').click(function(){
				...
				$.ajax({
				    ...
				    success:function(result){
				    	...
				    	if(result.success){
							layer.msg("用户信息修改成功！",{time:2000,icon:6,shift:5},function(){ 
							});
							window.location.href="${APP_PATH}/user/index";
				    	}else{
							layer.msg("用户信息修改失败！",{time:2000,icon:5,shift:5},function(){ 
							});	
				    	}
				    }
				});
